...
blr_wolf <- bayesglm(wolf ~ timebin, data = df, family = "binomial")
display(blr_wolf)

blr_rrh <- bayesglm(rrh ~ timebin, data = df, family = "binomial")
display(blr_rrh)
...
brm_wolf <- brm(wolf ~ timebin, data = df,
                ...
                family = "bernoulli", iter = 10000, sample_prior = TRUE)
summary(brm_wolf)
plot(brm_wolf)
...
plot(marginal_effects(brm_wolf, "timebin"))[[1]] +
    scale_x_discrete(
        name ="Time period", 
        limits = c("1800-1850","1850-1900","1900-1950","1950-2000")) +
    scale_y_continuous(
        name = "Probability of definite reference",
        labels = percent,
        limits = c(0, 1)) +
    theme_bw() + 
...
plot(marginal_effects(brm_rrh, "timebin"))[[1]] +
    scale_x_discrete(
        name ="Time period", 
        limits = c("1800-1850","1850-1900","1900-1950","1950-2000")) +
    scale_y_continuous(
